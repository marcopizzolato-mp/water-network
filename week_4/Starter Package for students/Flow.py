...
class FlowRaster(Raster):
    ...
    def getNeighbours(self, r, c):
        ...
        for i in range(8):
            ...
            # if its up again te edge it will kep the same value - to be checked
            if (rr>-1 and rr<self.getRows() and cc>-1 and cc<self.getCols()):
                neighbours.append(self._data[rr,cc])
                
        return neighbours
    # identify the lowest neighbors
    ...
    # Set the lowest neighbor
    #def setDownCells(self):
     #   lowestN=[]
      #  for r in range (self.getRows()): # get the rows 
       #     for c in range (self.getCols()): # get the cols
        #        lowestN = self.lowestNeighbour(r,c)
                # now if - else loop to set the 
