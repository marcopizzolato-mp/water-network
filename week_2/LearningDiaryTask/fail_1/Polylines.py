...
# The initialisation methods used to instantiate an instance
    def __init__(self,arg=None):
      #if what i'm passing is a list
      if isinstance(arg, list) or isinstance(arg, tuple):
          self._allPoints = []
          # ensure all the points are point2D
          for point in arg:
              if isinstance(point, Point2D):
                  self._allPoints.append(point.clone ())
      # passing in only one point
...
### TASK WEEK 2 ###
    ...
    def generalize(self, t):
        ...
        # Check wehter I do have moer than two points.
        # If I have less I do not need to run the semplification         
        if (self.size()<3):
    ...
    # gives the size of the list of points in self.allpoints      
    def size(self):
