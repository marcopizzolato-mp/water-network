    ...
    def calculateLakes (self):
        ...
        for i in ListPits:
            ...
            if i.getLakeFlag() == False:
                ...
                while (len(self.getNeighbours(int(mylake._lakeNodes[-1].get_y()),int(mylake._lakeNodes[-1].get_x()))) < 8 and (mylake._lakeNodes[-1].getPitFlag() == True)):
                
                #while len(self.getNeighbours(int(mylake._lakeNodes[-1].get_y()),int(mylake._lakeNodes[-1].get_x()))) == 8 or ( ( len(self.getNeighbours(int(mylake._lakeNodes[-1].get_y()),int(mylake._lakeNodes[-1].get_x()))) < 8) and (mylake._lakeNodes[-1].getPitFlag() == True)):
                ...
                highWall = mylake.maxelements()      
                    ...
                    # if mylake._lakeNodes[highWall[-1]].getPitFlag == False: # NO PERCHE SE LA CELLA PIU ALTA E' LA PITFLAG NON E' un LAGO
                #mylake.setHighWall(mylake._lakeNodes[highWall[-1]]) #i might get more highwalls.. in this case I'll pick the second
                # wrong #mylake.setLakeDownnode(mylake._lakeNodes[highWall[-1]+1])                
